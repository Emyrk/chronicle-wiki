            ...
            return (
              <Link key={server.slug} to={`/${server.slug}`} className="wiki-card group overflow-hidden border-border/60 bg-card transition hover:border-primary/50 hover:bg-black/70">
                <div className="relative h-28">
                  <img src={metadata.branding.bannerUrl} alt="" className="h-full w-full object-cover opacity-65 transition group-hover:opacity-85" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-black/55 to-transparent" />
                  <img src={metadata.branding.logoUrl} alt="" className="absolute bottom-3 left-3 h-12 w-12 rounded-lg border border-border/60 bg-black/70 object-cover" />
                </div>
                <div className="p-4">
