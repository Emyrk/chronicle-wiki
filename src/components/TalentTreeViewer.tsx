...
function iconUrl(texture: string) {
  return `https://icons.chronicleclassic.com/icons/${texture.toLowerCase()}.webp`;
}

function TalentButton({ talent, rank, onChange }: { talent: TalentEntry; rank: number; onChange: (rank: number) => void }) {
  ...
  return (
    ...
    >
      <img src={iconUrl(talent.iconTexture)} alt="" className={cn("h-full w-full rounded object-cover", rank === 0 && "grayscale opacity-45")} />
      <span className={cn(
...
function TalentTab({ tab, ranks, onRankChange }: { tab: TalentTabData; ranks: TalentRanks; onRankChange: (talent: TalentEntry, rank: number) => void }) {
    ...
    <section className="wiki-card p-4">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <img src={iconUrl(tab.iconTexture)} alt="" className="h-10 w-10 rounded border border-white/10" />
          <div>
          ...
          Array.from({ length: 4 }).map((__, col) => {
            ...
            return (
              <div key={`${tier}-${col}`} className="flex h-14 items-center justify-center">
                {talent && <TalentButton talent={talent} rank={ranks[talent.id] ?? 0} onChange={(rank) => onRankChange(talent, rank)} />}
              </div>
            );
          }),
        )}
...
export function TalentTreeViewer({ data }: { data: ClassTalentData }) {
